...
export default function AuthPage() {
  ...
  function getFirebaseError(err: unknown): string {
    ...
    if (msg.includes("user-not-found")||msg.includes("invalid-credential")||msg.includes("INVALID_LOGIN_CREDENTIALS"))
      return "No account found with this email and password. Please check your details or create an account.";
    ...
    if (msg.includes("popup-blocked")) return "Popup was blocked. Please allow popups for this site and try again.";
    return msg.replace("Firebase: ","").replace(/\s*\(auth\/.*?\)\.?/,"").trim();
  }

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (mode==="signup"&&!displayName.trim()) { showToast("Please enter your full name.","error"); return; }
    setLoading(true);
    try {
      if (mode==="signup") {
        await registerWithEmail(email, password);
        const firstName = displayName.trim().split(" ")[0];
        showToast(`🎉 Welcome, ${firstName}! Your account has been created successfully. Please verify your email.`,"success");
  ...
  function switchMode(newMode: "signin"|"signup") {
    setMode(newMode); setEmail(""); setPassword(""); setDisplayName(""); setPhone(""); setToast(null);
  }
  ...
  const CSS = `
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .form-panel{flex:none;width:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:40px 24px;position:relative;z-index:1}
    @media(min-width:960px){.form-panel{width:480px}}
    .back-link{display:flex;align-items:center;gap:8px;color:rgba(245,239,245,.3);font-size:12px;font-weight:300;letter-spacing:.5px;text-decoration:none;cursor:pointer;background:none;border:none;font-family:'Jost',sans-serif;padding:0;margin-bottom:32px;transition:color .2s;align-self:flex-start}
    .back-link:hover{color:rgba(200,164,196,.7)}
    ...
    .field-group{display:flex;flex-direction:column;gap:11px;margin-bottom:20px}
    ...
    .optional-badge{font-size:9px;letter-spacing:1px;color:rgba(245,239,245,.2);margin-left:6px;font-weight:300;text-transform:uppercase}
    .submit-btn{width:100%;padding:14px 20px;background:linear-gradient(135deg,rgba(200,164,196,.9),rgba(164,140,180,.9));border:none;border-radius:10px;color:#0a0608;font-family:'Jost',sans-serif;font-size:12px;font-weight:500;letter-spacing:2.5px;text-transform:uppercase;cursor:pointer;transition:all .25s;margin-bottom:18px}
