...
export default function GuestInvitePage() {
  ...
  return <main style={{ minHeight:"100vh", background:"linear-gradient(180deg,#fdf2f8,#eef2ff)", padding:24, display:"flex", justifyContent:"center", alignItems:"center" }}><div style={{ maxWidth:760, width:"100%", background:"#fff", borderRadius:20, padding:28, border:"1px solid #e5e7eb" }}><h1 style={{ fontSize:40, color:"#312e81" }}>You're Invited 🎉</h1><p>Hi {guestName}, {parentName} invited you to the reveal party.</p><p style={{ fontWeight:700 }}>{countdown}</p>{loading ? <p>Loading invite…</p> : done ? <><p style={{ color:"#15803d" }}>Thanks! Your prediction and message are already saved.</p><p style={{ marginTop:8 }}><strong>Prediction:</strong> {prediction === "boy" ? "Boy 💙" : prediction === "girl" ? "Girl 🩷" : "—"}</p>{message ? <p style={{ marginTop:6 }}><strong>Your note:</strong> {message}</p> : null}</> : <><div style={{ display:"flex", gap:10, marginTop:8 }}><button onClick={()=>setPrediction("boy")} style={{ flex:1, padding:14, borderRadius:12, border:"1px solid #ddd", background: prediction==="boy"?"#dbeafe":"#fff" }}>💙 Boy</button><button onClick={()=>setPrediction("girl")} style={{ flex:1, padding:14, borderRadius:12, border:"1px solid #ddd", background: prediction==="girl"?"#fce7f3":"#fff" }}>🩷 Girl</button></div><textarea placeholder="Write a message for the parents…" value={message} onChange={(e)=>setMessage(e.target.value)} style={{ width:"100%", marginTop:12, minHeight:100, border:"1px solid #ddd", borderRadius:12, padding:10 }} /><button onClick={submit} disabled={!prediction||submitting} style={{ marginTop:12, width:"100%", padding:12, border:"none", borderRadius:12, background:"#2563eb", color:"#fff" }}>{submitting?"Submitting...":"Submit prediction & message"}</button></>}{error && <p style={{ color:"#b91c1c" }}>{error}</p>}</div></main>;
}
